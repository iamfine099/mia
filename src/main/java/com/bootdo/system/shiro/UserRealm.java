...
//    }
     ...
     * 支持的登陆类型
     */
    private String supportedLoginType = "admin";
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		//map.put("fType",new String[] {Constant.USER_TYPE_ADMIN});
		...
		// 账号不存在
		if (user == null) {
			throw new UnknownAccountException("账号或密码不正确1");
		}

		// 密码错误
		if (!password.equals(user.getPassword())) {
			throw new IncorrectCredentialsException("账号或密码不正确2");
		}
