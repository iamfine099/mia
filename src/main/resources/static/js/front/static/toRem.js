(function(c, f) {
				...
				if(!a && !r) {
					...
					if(o) { if(k >= 3 && (!a || a >= 3)) { a = 3 } else { if(k >= 2 && (!a || a >= 2)) { a = 2 } else { a = 1 } } b.setAttribute("class", "ios") } else {
						a = 1;
				...
				if(!m) {
					...
					if(b.firstElementChild) { b.firstElementChild.appendChild(m) } else {
				...
				if(s.readyState === "complete") { s.body.style.fontSize = 12 * a + "px" } else { s.addEventListener("DOMContentLoaded", function(t) { s.body.style.fontSize = 12 * a + "px" }, false) } i();
				d.dpr = c.dpr = a;
				d.refreshRem = i;
				d.rem2px = function(u) { var t = parseFloat(u) * this.rem; if(typeof u === "string" && u.match(/rem$/)) { t += "px" } return t };
				d.px2rem = function(u) { var t = parseFloat(u) / this.rem; if(typeof u === "string" && u.match(/px$/)) { t += "rem" } return t }
