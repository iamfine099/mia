			...
			.bootstrapTable(
					{
						...
						url : prefix + "/articleReviewList", // 服务器数据的加载地址
					//	showRefresh : true,
					//	showToggle : true,
					//	showColumns : true,
						iconSize : 'outline',
...
function edit(id) {
	var index = layer.open({
		...
		shadeClose : false, // 点击遮罩关闭层
		area : [ '800px', '520px' ],
		content : prefix + '/articleReviewEdit/' + id // iframe的url
	});
	layer.full(index);
}
function remove(id) {
...
function checkInfo(){
	...
	var index = layer.open({
		...
		shadeClose : false, // 点击遮罩关闭层
		area : [ '800px', '65%' ],
		content : prefix + '/articleReviewCheckInit?ids='+ids // iframe的url
	});
	layer.full(index);
}