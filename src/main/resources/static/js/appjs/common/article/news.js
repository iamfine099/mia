			...
			.bootstrapTable(
					{
						...
						url : prefix + "/newsList", // 服务器数据的加载地址
					//	showRefresh : true,
					//	showToggle : true,
					//	showColumns : true,
						iconSize : 'outline',
						...
						columns : [
								{
									...
									formatter : function(value, row, index) {
										return getDictLabelByValue(newsTypeJson,value);
									}  
								},
																{
									field : 'createTime', 
									...
									title : '状态',
									formatter : function(value, row, index) {
										return getDictLabelByValue(showHideJson,value);
									}
								},
																{
									title : '操作',
									...
									formatter : function(value, row, index) {
										...
										var ut = '<a class="btn btn-primary btn-sm '+s_edit_h+'" href="#" title="置顶"  mce_href="#" onclick="updateTop(\''
												+ row.id
												+ '\')"><i class="fa fa-arrow-up"></i></a> ';
										var ud = '<a class="btn btn-primary btn-sm '+s_edit_h+'" href="#" title="取消置顶"  mce_href="#" onclick="updateDown(\''
												+ row.id
...
function add() {
	layer.open({
		...
		area : [ '800px', '520px' ],
		content : prefix + '/newsAdd' // iframe的url
	});
}
function edit(id) {
	layer.open({
		...
		shadeClose : false, // 点击遮罩关闭层
		area : [ '800px', '520px' ],
		content : prefix + '/newsEdit/' + id // iframe的url
	});
}
function remove(id) {
