			...
			.bootstrapTable(
					{
						...
						url : prefix + "/articleRecommendList", // 服务器数据的加载地址
					//	showRefresh : true,
					//	showToggle : true,
					//	showColumns : true,
						iconSize : 'outline',
						...
						queryParams : function(params) {
							...
							searchForm['status']=1,
							searchForm['articleType']='recommend'
							return searchForm;
						},
						...
						columns : [
								{
									...
									formatter : function(value, row, index) {
										...
										var r = '<a class="btn btn-primary btn-sm '+s_edit_h+'" href="#" mce_href="#" title="推荐" onclick="operation(\''
												+ row.id 
												+ '\',1,1)"><i class="fa fa-thumbs-up"></i></a> ';
										var nr = '<a class="btn btn-primary btn-sm '+s_edit_h+'" href="#" mce_href="#" title="取消推荐" onclick="operation(\''
												+ row.id 
												+ '\',1,0)"><i class="fa fa-thumbs-down"></i></a> ';
										var t = '<a class="btn btn-primary btn-sm '+s_edit_h+'" href="#" mce_href="#" title="置顶" onclick="operation(\''
												+ row.id 
												+ '\',2,1)"><i class="fa fa-arrow-up"></i></a> ';
										var nt = '<a class="btn btn-primary btn-sm '+s_edit_h+'" href="#" mce_href="#" title="取消置顶" onclick="operation(\''
												+ row.id 
												+ '\',2,0)"><i class="fa fa-arrow-down"></i></a> ';
...
function add() {
	layer.open({
		...
		area : [ '800px', '520px' ],
		content : prefix + '/noticeAdd' // iframe的url
	});
}
function edit(id) {
	var index = layer.open({
		...
		shadeClose : false, // 点击遮罩关闭层
		area : [ '800px', '520px' ],
		content : prefix + '/articleRecommendEdit/' + id // iframe的url
	});
	layer.full(index);
}
function remove(id) {
...
function operation(id,type,val){
