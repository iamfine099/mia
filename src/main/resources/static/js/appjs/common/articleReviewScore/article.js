			...
			.bootstrapTable(
					{
						...
						url : prefix + "/articleReviewScoreList", // 服务器数据的加载地址
					//	showRefresh : true,
					//	showToggle : true,
					//	showColumns : true,
						iconSize : 'outline',
						...
						columns : [
									{
										...
										formatter : function(value, row, index) {
											return getDictLabelByValue(ifCheckedJson,value);
										}
									},
																	{
...
function edit(id) {
	var index = layer.open({
		...
		shadeClose : false, // 点击遮罩关闭层
		area : [ '800px', '520px' ],
		content : prefix + '/articleReviewScoreEdit/' + id // iframe的url
	});
	layer.full(index);
}
function remove(id) {
...
function checkInfo(){
	...
	var index = layer.open({
		...
		shadeClose : false, // 点击遮罩关闭层
		area : [ '800px', '65%' ],
		content : prefix + '/articleReviewScoreCheckInit?ids='+ids // iframe的url
	});
	layer.full(index);
}